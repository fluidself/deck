...
export const encryptData = async (data: any, keys: undefined | string | KeyPair, sea: any) => {
  return keys && sea ? sea.encrypt(data, keys) : Promise.resolve(data);
};

export const decryptData = async (data: any, keys: undefined | string | KeyPair, sea: any) => {
  return keys && sea ? sea.decrypt(data, keys) : Promise.resolve(data);
};

export const debouncedUpdates = (dispatcher: any, type = 'object', timeout = 100) => {
  let updates: any[] = [];
  let handler: any;
...
export const nodeReducer = <T>(state: NodeT<T>, { data, type }: ActionType<T>): T => {
  switch (type) {
    case 'add':
      return { ...state, ...data };
    case 'remove':
...
export const collectionReducer = <T>(state: CollectionState<T>, { data, type }: ActionType<T>): CollectionState<T> => {
  switch (type) {
    case 'add':
...
export const useSafeReducer = <T>(reducer: any, initialState: T): [T, Function] => {
  const [state, dispatch] = useReducer<React.Reducer<T, ActionType<T>>>(reducer, initialState);
...
export const useGunKeyAuth = (gun: GunRef, keys: KeyPair, triggerAuth: boolean = true) => {
...
export const useGunKeys = (sea: any, existingKeys?: KeyPair | undefined | null) => {
  const [newKeys, setNewKeys] = useState<KeyPair | undefined | null>(existingKeys);
...
export const useGunOnNodeUpdated = <T>(
  ...
  useEffect(() => {
    if (isMounted.current) {
      const gunCb = async (encryptedField: any, nodeID: string, message: any, event: any) => {
...
export const useGunState = <T>(
  ...
  const put = async (data: T) => {
    ...
    await new Promise((resolve, reject) =>
      gunAppGraph.put(encryptedData, (ack: any) => (ack.err ? reject(ack.err) : resolve(data))),
    );
  };

  const remove = async (field: string) => {
    await new Promise((resolve, reject) => gunAppGraph.put(null, (ack: any) => (ack.err ? reject(ack.err) : resolve(field))));
...
export const useGunCollectionState = <T>(
  ...
  const { appKeys, sea, interval = 100 } = opts;
  const [gunAppGraph] = useState(ref);
  const [{ collection }, dispatch] = useSafeReducer<CollectionState<T>>(collectionReducer, {
    collection: new Map(),
  });
  ...
  const updateInSet = async (nodeID: string, data: T) => {
    let encryptedData = await encryptData(data, appKeys, sea);
    await new Promise((resolve, reject) =>
      gunAppGraph.get(nodeID).put(encryptedData, (ack: any) => (ack.err ? reject(ack.err) : resolve(data))),
    );
  ...
  const addToSet = async (data: T, nodeID?: string) => {
    ...
    if (!nodeID) {
      await new Promise((resolve, reject) =>
        gunAppGraph.set(encryptedData, (ack: any) => (ack.err ? reject(ack.err) : resolve(data))),
      );
    } else {
      await new Promise((resolve, reject) =>
        gunAppGraph.get(nodeID).put(encryptedData, (ack: any) => (ack.err ? reject(ack.err) : resolve(data))),
  ...
  const removeFromSet = async (nodeID: string) => {
    await new Promise((resolve, reject) =>
      gunAppGraph.get(nodeID).put(null, (ack: any) => (ack.err ? reject(ack.err) : resolve(nodeID))),
    );
  };
